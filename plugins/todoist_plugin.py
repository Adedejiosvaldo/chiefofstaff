...
def setup_plugin(registry):
    registry.register_tool(
        ...
        func=fetch_todoist_tasks,
        description="Fetches active and overdue tasks from Todoist. Can accept a filter string (defaults to 'today | overdue'). Returns mock lists if TODOIST_API_TOKEN is unconfigured.",
    )
    registry.register_tool(
        name="create_todoist_task",
        func=create_task,
        description="Creates a new task in Todoist. Parameters: content (task name), due_string (natural date text e.g., 'today', 'monday 4pm').",
    )
    registry.register_tool(
        name="complete_todoist_task",
        func=complete_task,
        description="Marks a specific Todoist task as completed using its alphanumeric ID.",
    )
